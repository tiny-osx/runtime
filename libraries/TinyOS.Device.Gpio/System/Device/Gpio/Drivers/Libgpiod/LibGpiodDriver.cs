...
namespace System.Device.Gpio.Drivers.Libgpiod;

/// <summary>
/// Driver that uses libgpiod V2 for GPIO control.
/// </summary>
//[Experimental(DiagnosticIds.SDGPIO0001, UrlFormat = DiagnosticIds.UrlFormat)]
internal sealed class LibGpiodDriver : UnixDriver
{
    private static readonly string ConsumerId = $"C#-{nameof(LibGpiodDriver)}-{Process.GetCurrentProcess().Id}";
    ...
    private readonly LibGpiodEventObserver _eventObserver;
    ...
    public LibGpiodDriver(Chip chip, TimeSpan? waitEdgeEventsTimeout = null)
    {
        _chip = chip;
        _eventObserver = new LibGpiodEventObserver { WaitEdgeEventsTimeout = waitEdgeEventsTimeout ?? TimeSpan.FromMilliseconds(100) };
    }
    ...
    {
        throw new NotSupportedException($"{nameof(LibGpiodDriver)} uses GPIO line numbering. For more information please refer to " +
    ...
    protected internal override WaitForEventResult WaitForEvent(int lineOffset, PinEventTypes eventTypes, CancellationToken cancellationToken)
    {
        ...
        var eventSubscription = new LibGpiodEventObserver.EventSubscription(offset, Translator.Translate(eventTypes));

        LibGpiodEventObserver.EventWaitHandle eventWaitHandle = _eventObserver.ObserveSingleEvent(lineRequest, eventSubscription);
    ...
    protected internal override void AddCallbackForPinValueChangedEvent(int lineOffset, PinEventTypes eventTypes, PinChangeEventHandler callback)
    {
        ...
        lock (_lockObject)
        {
            if (_requestedLineByLineOffset.TryGetValue(offset, out var requestedLines))
            {
                ...
                _eventObserver.Observe(request, new LibGpiodEventObserver.EventSubscription(offset, Translator.Translate(eventTypes)),
                    callback);
            }
        }
    }
